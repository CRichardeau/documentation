...
- :guilabel:`Paper format`: This defines the default paper size of reports. You can select
  ...
  .. note::
     Other paper formats may be available depending on which apps or modules you have installed.
     For example, label sheets for the Inventory app or event badges for the Events app.
...
(e.g., sales orders) access the model, click the **Toggle Studio** button, then click
...
**Toggle Studio** button, then click :guilabel:`Reports`. Select an existing report to open it.
...
- Change the :guilabel:`Report name`: The new name is applied everywhere (in Studio, in the
  :guilabel:`Print` menu under the :icon:`fa-cog` (:guilabel:`gear`) icon in the record, and in the
  PDF file name).
...
- :guilabel:`Show in print menu`: to add the report to the record's :guilabel:`Print` menu.
