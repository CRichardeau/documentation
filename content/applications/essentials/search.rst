...
values <search/values>`, or click :icon:`fa-caret-down` (**dropdown**) to access the :ref:`Filter
...
for, and select the desired option from the drop-down menu to apply the search filter.
...
.. example::
   ...
   *Mitchell Admin* is the salesperson on the *Sales Analysis* report (:menuselection:`Sales app -->
   Reporting --> Sales`), search for `Mitch`, and click :icon:`fa-caret-right` (**submenu**) next to
...
.. note::
   ...
   selected (without selecting :icon:`fa-caret-right` (**submenu**), *all* records containing the
...
Modify the default selection of records by clicking :icon:`fa-caret-down` (**dropdown**) from the
...
.. example::
   ...
   :icon:`fa-times` (**cancel**), then select :menuselection:`Order Date --> 2024`.
...
custom filter. To do so, click :icon:`fa-caret-down` (**dropdown**) in the search bar, then select
...
To save the current view as a favorite, click :icon:`fa-caret-down` (**dropdown**) in the search
...
Saved favorites can be accessed by clicking :icon:`fa-trash` (**delete**) in the search bar, then
selecting the saved filter in the :guilabel:`Favorites` drop-down menu. To remove a saved favorite,
click :icon:`fa-trash` (**delete**) next to the favorited search.
